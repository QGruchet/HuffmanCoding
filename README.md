...
For the second part, we have to translate a text to binary code using Hauffman Encodation. The CPP code was not very difficult. For tree we have just change type node data, is not a single integer it's now a struct with a integer and a char.  After we have add a new file, the parser file, which read the file, calculate all frequencies of char, create hufftree with this frequencies, read the huffman tree, and return the convert text. Also we wanted to upgrade graphic interface. This spend more of our time because Qt is difficult to learn. The new interface consist of a main menu, encoding menu, decoding menu and some other thing, like present in "usage". We had some new classes, like MyButton for manipulate our special button, with icon and note text.  More precisely we have replace text by icon and add information text we the user use his mouse to fly over the icon for more clarity and  aesthetic.
On top of that we add MyTextEdit file for manipulation text zone. we had text in grey for show where user need to write and where the translation is print. We follow of modèle of "google translate" which is enough soft to be programming in few weeks.
...
- Our coding does not support accentued characters or special characters.
